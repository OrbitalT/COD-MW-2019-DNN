...
while 1 == 1:
    ...
    image = pyautogui.screenshot(
        region=(centerPoint[0] - 100, centerPoint[1] - 100, 200, 200))
    # Format and Normalize Data
    normalizedImage = np.asarray([np.asarray(image)]) / 255
    # Save
    baseDirectory = 'C:\\Users\\bryce\\Desktop\\COD\\EnemyDetector\\SC'
    generatedGUID = str(uuid.uuid1())
    image.save(baseDirectory + generatedGUID + '.png', 'png')
    print('Done!')
    time.sleep(1)
